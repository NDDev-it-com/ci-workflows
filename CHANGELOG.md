...
- Add bounded real Flutter, Android/Kotlin and Qt fixture projects with exact
  hosted toolchains, generated byte provenance, independent runtime observers,
  structured build/test receipts and tier-neutral Gradle caching. A cold-process
  gate now syntax-checks, imports and executes every Python validator from a
  sanitized unrelated working directory, rejecting undefined dependencies and
  import-time side effects before they can false-green the aggregate gate (#129).
