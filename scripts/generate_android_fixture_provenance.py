...
from _gradle_lockfile import GradleLockfileError, parse_gradle_95_lockfile
...
def _environment(home: Path) -> dict[str, str]:
    return clean_environment(
        {"CI": "true", "GRADLE_USER_HOME": str(home)},
        inherit=("ANDROID_HOME", "ANDROID_SDK_ROOT", "JAVA_HOME"),
    )
...
def _java_identity(executable: Path | str, home: Path) -> dict[str, str]:
    output = subprocess.run(
        ...
        env=_environment(home), text=True, stdout=subprocess.PIPE,
...
def _versions(root: Path, home: Path) -> dict[str, str]:
    ...
    expected_launcher_details = f'{java["java.vendor"]} {java["java.runtime.version"]}'
    if launcher_version != java["java.version"] \
            or launcher_details != expected_launcher_details \
            or Path(daemon["java.home"]).resolve() != Path(java["java.home"]).resolve() \
            or any(daemon[key] != java[key] for key in (
                "java.version", "java.runtime.version", "java.vendor", "java.vm.name"
            )):
        raise RuntimeError("Gradle launcher/daemon JVM does not match observed build Java")
    return {
